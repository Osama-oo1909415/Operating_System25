...
# Deploy public key to dev_lead1's authorized_keys
sudo cp "$PUB_KEY" "$AUTHORIZED_KEYS"
sudo -u dev_lead1 chmod 600 "$AUTHORIZED_KEYS"
sudo -u dev_lead1 chown -R dev_lead1:dev_lead1 "$KEY_DIR"
...
# Use sshpass to automate SCP password input
echo "Copying private key to VM2 and VM3..."
sshpass -p '123' scp "$TEMP_KEY" "vm2@$VM2_IP:/tmp/" > /dev/null 2>&1 && echo "Private key copied to VM2." || echo "Failed to copy to VM2."
sshpass -p '123' scp "$TEMP_KEY" "vm3@$VM3_IP:/tmp/" > /dev/null 2>&1 && echo "Private key copied to VM3." || echo "Failed to copy to VM3."

# Disable password authentication for dev_lead1
SSHD_CONFIG="/etc/ssh/sshd_config"

if ! grep -q "Match User dev_lead1" "$SSHD_CONFIG"; then
    echo -e "\nMatch User dev_lead1\n    PasswordAuthentication no\n    PubkeyAuthentication yes\n    AuthorizedKeysFile      .ssh/authorized_keys" | sudo tee -a "$SSHD_CONFIG" > /dev/null
